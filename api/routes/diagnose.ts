...
const openai = new OpenAI({
  apiKey: process.env.LITELLM_API_KEY || 'sk-fzyL4lLBBlU8DxRiXq7Ymg',
...
你的核心能力是基于【子午流注辩证方法】进行疾病定性、原因辩证分析和处置方案推荐。
...
3. 第三步：处置方案推荐。包括临床处方（穴位、方剂）、生活处方（作息、饮食）、运动处方。
...
请以JSON格式输出结果，结构如下：
{
  ...
  "treatment_recommendations": {
    "clinical": [
      { "type": "穴位", "name": "穴位名", "method": "操作法", "reason": "选取理由" },
      { "type": "方剂", "name": "方名", "instruction": "加减建议", "reason": "选取理由" }
    ],
...
`;
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-5', // 使用 LiteLLM 提供的 gpt-5 模型
      messages: [
